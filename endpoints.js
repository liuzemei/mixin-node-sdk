...
class MixinBase extends Mixin {
  constructor(config, optionsOrUseChinaServer, debug) {
    super(config);
    this.CLIENT_CONFIG = config
    const options = optionsOrUseChinaServer && typeof optionsOrUseChinaServer === 'object' ? optionsOrUseChinaServer : {
      useChinaServer: optionsOrUseChinaServer,
      debug
    };
    this._request = _request(config, options)
  }
